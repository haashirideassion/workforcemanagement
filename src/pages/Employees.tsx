...
import { MagnifyingGlass, Plus, Funnel, DotsThree } from '@phosphor-icons/react';
...
function getUtilizationBadge(utilization: number) {
    if (utilization >= 80) {
        ...
        return <Badge className="bg-red-100 text-red-700 hover:bg-red-100">Risk</Badge>;
    }
}
...
export function Employees() {
    ...
    const handleCreateEmployee = (values: EmployeeFormData) => {
        console.log('Creating employee:', values);
        // TODO: Call useCreateEmployee mutation when connected to Supabase
        setFormOpen(false);
    };
                        ...
                        <TableBody>
                            {filteredEmployees.map((employee) => (
                                <ContextMenu key={employee.id}>
                                        ...
                                        >
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    <Progress value={employee.utilization} className="h-2 w-20" />
                                                    <span className="text-sm text-muted-foreground">{employee.utilization}%</span>
                                                </div>
                                            </TableCell>
                                            <TableCell>{getUtilizationBadge(employee.utilization)}</TableCell>
                                            <TableCell>
                                                <DropdownMenu>
                                    ...
                                    <ContextMenuContent>
                                        <ContextMenuItem onSelect={() => navigate(`/employees/${employee.id}`)}>
                                            View Details
                                        </ContextMenuItem>
                                        <ContextMenuItem>Edit Employee</ContextMenuItem>
                                        <ContextMenuItem>Assign to Project</ContextMenuItem>
                                        <ContextMenuSeparator />
                                        <ContextMenuItem className="text-red-500">Deactivate</ContextMenuItem>
                                    </ContextMenuContent>
                                </ContextMenu>
                            ))}
                        </TableBody>
